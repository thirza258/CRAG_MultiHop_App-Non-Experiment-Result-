...
class ExternalSearcher:
    def __init__(self, config):
        ...
        self.top_k = config.get("external_top_k", 5)
    ...
    def _get_embeddings(self, texts: List[str], batch_size: int = 100) -> List[List[float]] | None:
        ...
        for i in range(0, len(cleaned_texts), batch_size):
            ...
            try:
                ...
                logger.info(f"Error fetching embeddings for batch {i // batch_size}: {e}")
                return None 
    ...
    def search_news(self, query: str) -> tuple[List[str], List[Dict]]:
        ...
        if not news_api_key:
            logger.info("Error: NEWS_API_KEY not found in environment variables.")
            return [], []
        ...
        try:
            ...
            logger.info(f"News search error: {e}")
            return [], []
